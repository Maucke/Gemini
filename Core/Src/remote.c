...
//#if GEMINI == 1
	...
	TIM5_CH2Config.ICSelection=TIM_ICSELECTION_DIRECTTI;//映射到TI4上
	TIM5_CH2Config.ICPrescaler=TIM_ICPSC_DIV1;          //配置输入分频，不分频
	TIM5_CH2Config.ICFilter=0x03;                       //IC4F=0003 8个定时器时钟周期滤波
	HAL_TIM_IC_ConfigChannel(&TIM5_Handler,&TIM5_CH2Config,TIM_CHANNEL_2);//配置TIM5通道4
...
void HAL_TIM_IC_MspInit(TIM_HandleTypeDef *htim)
{
    ...
    GPIO_Initure.Pin=GPIO_PIN_1;            //PB9
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)//捕获中断发生时执行
{
	if(htim->Instance==TIM5)
	{
		if(RDATA)//上升沿捕获
		{
			...
			TIM_SET_CAPTUREPOLARITY(&TIM5_Handler,TIM_CHANNEL_2,TIM_ICPOLARITY_FALLING);//CC4P=1 设置为下降沿捕获
			__HAL_TIM_SET_COUNTER(&TIM5_Handler,0);  	//清空定时器值   	  
		  	RmtSta|=0X10;							//标记上升沿已经被捕获
		}else //下降沿捕获
		{
			Dval=HAL_TIM_ReadCapturedValue(&TIM5_Handler,TIM_CHANNEL_2);//读取CCR4也可以清CC4IF标志位
			TIM_RESET_CAPTUREPOLARITY(&TIM5_Handler,TIM_CHANNEL_2);   	//一定要先清除原来的设置！！
			TIM_SET_CAPTUREPOLARITY(&TIM5_Handler,TIM_CHANNEL_2,TIM_ICPOLARITY_RISING);//配置TIM5通道4上升沿捕获
...
{        
	...
	{ 
		...
		if((t1==REMOTE_ID1)&&(t2==REMOTE_ID2))//检验遥控识别码(ID)及地址
		{ 
				t1=RmtRec>>8;
				t2=RmtRec; 	
				if(t1==(u8)~t2)
